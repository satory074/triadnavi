...
- `core/appState.ts` / `core/presets.ts` — アプリの状態、保存データの検証つき読み込み、対局中のルール変更(`changeRules`)、デッキ名の変更(`renameDeck`)
...
- **「設定に戻る」は対局中なら 2 段階**: 記録を閉じる操作なので、置いたカードや開いたカードがあれば `ConfirmAction` で「対局を閉じて戻る」を挟む。終局後と 1 枚も置いていない時はそのまま戻る。「対戦を始める」と再戦の開始では先頭にスクロールする(設定画面の途中の位置のまま対局画面が出ていた)。
- **Enter は「おすすめ通りに打った」**: ボタンが出ている時だけ効く。ダイアログが開いている時(`dialog[open]`)と文字の入力中は効かない。マウスで押したボタンにはフォーカスが残るので、`preventDefault()` でそのボタンが Enter でもう一度押されるのを止めている。
...
- **ロゴは対戦前の画面へのリンク**: 手持ちの画面を閉じ、対局中なら「設定に戻る」と同じく対局の記録を閉じる(設定の下書きは残る)。`<a href={BASE_URL}>` なので、Ctrl/⌘ + クリックは新しいタブで開く。
