...
- **所有率とアチーブメントは手持ちから判定する**: 手持ちの画面に「所有率」(同梱データのカード全体に対する所持枚数の割合。小数 1 桁で切り捨て。四捨五入だと 474 / 475 枚で 100.0% になる)と、達成したアチーブメントを出す。アプリが知っているのは所持カードだけなので、判定できる 2 種類だけを扱う: 「カードコレクター：ランク1〜14」(カードを N 種類入手する。Ex. も数える)と「生粋のカードマニア」(No.1〜No.312 をすべて入手する)。NPC への勝利・大会・ルーレットの系統は記録が無いので出さない。データは `fetch-data.mjs` が FFXIV Collect のアチーブメントから説明文の形で選んで `achievements.json` に書く。文言が変わって件数が減ったら例外で止まる(黙って空にならないように)。
